...
pyflakes:
	find . \( -name _build -o -name var -o -path ./docs -o -path ./bit/contrib \) -type d -prune -o -name '*.py' -print0 | $(XARGS) pyflakes

pep8:
	find . \( -name _build -o -name var \) -type d -prune -o -name '*.py' -print0 | $(XARGS) -n 1 pep8 --repeat --exclude=build/*,docs/*,bit/contrib* --ignore=E731,E402

test: clean
	py.test

coverage: clean
	py.test --cov-config .coveragerc --cov-report term-missing --cov=bit tests/
